...
def bytes_to_unicode():
    ...
    # Il s'agit de tous les caractères imprimables de ASCII et ISO-8859-1 sauf le trait d'union conditionnel (0xAD)
    # attention, bs contient des entiers, et pas des caractères.
    ...
    # on transforme les entiers de cs en les caractères correspondants.
    d = dict(zip(bs, cs))
    # et on fait un dico de caractères "affichables."
    return d
...
def get_encoder(langue=None):
    ...
    if langue==None:
        ...
        encoder_local_file = os.path.join(os.path.dirname(__file__), 'encoderFR.json')
        vocab_local_file = os.path.join(os.path.dirname(__file__), 'vocabFR.bpe')
        if not os.path.isfile(encoder_local_file):
            print(encoder_local_file)
            raise FileNotFoundError
        if not os.path.isfile(vocab_local_file):
            print(vocab_local_file)
            raise FileNotFoundError
        with open(encoder_local_file, 'rb') as f:
            encoder = json.load(f)
        with open(vocab_local_file, 'r', encoding="utf-8") as f:
            bpe_data = f.read()
        bpe_merges = [tuple(merge_str.split()) for merge_str in bpe_data.split('\n')[1:-1]]
...
class BPETokenizer:
    ...
    def __init__(self, langue=None):
        self.encoder = get_encoder(langue)
    ...
    def decode(self, idx, visible=True):
        # ensure a simple 1D tensor for now
        assert idx.ndim == 1
        # decode indices to text
        if visible:
            #print(idx.tolist())
            text = self.encoder.decode_visible(idx.tolist())
        else:
